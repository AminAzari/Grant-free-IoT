%IHN
function TTest_final(lambda,Nt,InSynT,InSynF,H )
 Eth=0;
X1=['tLa',num2str(lambda),'Nt',num2str(Nt),'InSynT',num2str(InSynT),'InSynF',num2str(InSynF),'H',num2str(H) ,'.mat'];
ef=exist(X1, 'file');
if (ef==0)
    X=['tLa',num2str(lambda),'Nt',num2str(Nt),'InSynT',num2str(InSynT),'InSynF',num2str(InSynF),'H',num2str(H) ];
%     save([X,'.mat'],'Nt')
    nzc=23;
    ncp=2;
...
%     Eth=0.1;
    ...
    cons=.1;
    ia=0;
    Nrep=floor(10000/(Tperiod*lambda));
...
%     save([X,'.mat'],'NSucRate','nNSucRate','SucRate','ConAl','Np')
end
